...
class FitnessFunction:
    def __init__(self, accuracy_weight=1.0, speed_weight=1.0, token_limit_weight=1.0,
                 ...
                 speed_limit=10.0,
                 token_hard_limit_threshold=1.0, token_soft_limit_threshold=0.9):
        ...
        self.token_limit_instance = Token_limit_ratio(hard_limit_threshold=token_hard_limit_threshold, soft_limit_threshold=token_soft_limit_threshold)
        self.speed_instance = Speed(speed_limit=speed_limit)
    
    def get_fitness_score(self, prompt_chain, final_prompt_output, elapsed_time):
        """
        ...
        """
        # Convert prompt_chain to list of (model_name, prompt_input) tuples
        phenotype_chain = [(step[0], ''.join(step[1:])) for step in prompt_chain]
        ...
        for model_name, prompt_input in phenotype_chain:
        ...
        fitness_score = (self.accuracy_weight * accuracy_score) * (speed_score ** self.speed_weight) * (token_limit_score ** self.token_limit_weight)
...
if __name__ == "__main__":
    ...
    prompt_chain = [("gpt-3.5-turbo", "This is ", "an input ", "prompt"), ("gpt-4", "This another input prompt")]
    # Phenotype chain: [(model_name, prompt_input), ...]
    phenotype_chain = [("gpt-3.5-turbo", "This is an input prompt"), ("gpt-4", "This another input prompt")]
    ...
    prompt_output_chain = run_prompt_chain(prompt_chain)

    total_elapsed_time = sum([time_taken for _, time_taken in prompt_output_chain])
    final_output = prompt_output_chain[-1][0]
    fitness_score = fitness_function.get_fitness_score(prompt_chain, final_output, total_elapsed_time)
